 ...
 */
package com.threetree.ttrefreshrecyclerview.viewimpl;
...
import com.threetree.ttrefreshrecyclerview.base.HTBaseRecyclerView;
import com.threetree.ttrefreshrecyclerview.base.HTOrientation;
import com.threetree.ttrefreshrecyclerview.base.HTViewHolderTracker;
import com.threetree.ttrefreshrecyclerview.base.HTWrapperAdapter;

import java.util.Arrays;

import static com.threetree.ttrefreshrecyclerview.utils.Utils.getFirstItemPosition;
...
abstract class HTBaseRecyclerViewImpl extends HTBaseRecyclerView {
    ...
    {
        if (mHTViewHolder == null || mLoadMoreContainerView == null || mLoadMoreDelegate == null || mHTWrapperAdapter == null || !mHTWrapperAdapter.hasLoadMoreView()) {
            return false;
        }
...
//                    hideLoadMoreView(true);
    ...
    public void startAutoRefresh()
    {
        if (mRecyclerView != null && mHTViewHolder != null && mHTViewHolder.getRefreshContainerView() != null) {
            ...
            if (layoutManager != null && layoutManager.getItemCount() > 0) {
                int pos = getFirstItemPosition(layoutManager, true);
    ...
    private int calculateChildrenSize(boolean includeLoadMore)
    {
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ...
            GridLayoutManager.SpanSizeLookup sizeLookup = ((GridLayoutManager) layoutManager).getSpanSizeLookup();
            int totalSpanSize = 0;
            int max=0;
            for (int k = 0, size = childCount; k < size; k++) {
                int spanSize = sizeLookup.getSpanSize(k);
                View child = layoutManager.getChildAt(k);
                if (child != null && (includeLoadMore || child != mLoadMoreContainerView)) {
                    ...
                    totalSpanSize = totalSpanSize + spanSize;
                    RecyclerView.LayoutParams lp = (RecyclerView.LayoutParams) child.getLayoutParams();
                    int space = checkOrientationVertical() ? layoutManager.getDecoratedMeasuredHeight(child) + lp.bottomMargin + lp.topMargin
                            : layoutManager.getDecoratedMeasuredWidth(child) + lp.leftMargin + lp.rightMargin;
                    if(k==size-2)
                    {
                        //最后的数据项是否需要补齐
                        if(spanSize%columns != 0)
                        {
                            max += space*columns;
                        }else
                        {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                ...
                if (child != null && (includeLoadMore || child != mLoadMoreContainerView)) {
                    ...
                    RecyclerView.LayoutParams lp = (RecyclerView.LayoutParams) child.getLayoutParams();
                    columnSizes[j] += checkOrientationVertical() ? layoutManager.getDecoratedMeasuredHeight(child) + lp.bottomMargin + lp.topMargin : layoutManager.getDecoratedMeasuredWidth(child) + lp.leftMargin + lp.rightMargin;
                }
            }
        }
    ...
    protected void changeLoadMoreViewPositionWithAnimation(int targetPosition, @Nullable Animator.AnimatorListener animatorListener)
    {
        ...
        if (startValue <= -mLoadMoreViewSize)
            return;
        if (mLoadMoreAnimator != null && mLoadMoreAnimator.isRunning()) {
