 ...
 * 水平方向刷新实现
 */
public class HTHorizontalRightRecyclerViewImpl extends HTHorizontalRecyclerViewImpl {

    public HTHorizontalRightRecyclerViewImpl(Context context) {
        super(context);
    }

    public HTHorizontalRightRecyclerViewImpl(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public HTHorizontalRightRecyclerViewImpl(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public Boolean handleMoveAction(MotionEvent event) {
        ...
        boolean canMoveLeft = mHTViewHolderTracker.hasLeftIdlePosition();

        if (moveRight && checkChildScroll()) {
            return defaultDispatchTouchEvent(event);
        }
        if (((moveLeft && canMoveLeft) || moveRight) && shouldHandleRefresh()) {
